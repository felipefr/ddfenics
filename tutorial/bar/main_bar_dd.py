...
database_file = 'database_generated.txt' # previous computed
...
problem = DDProblem(a, b, sol, [bcL], metric = DDMetric(ddmat.C, V = Sh0, dx = dx, omega = -0.5)) 

start = timer()
#replaces df.LinearVariationalSolver(problem)
solver = DDSolver(problem, opInit = 'zero', seed = 1)
tol = 1e-7
