...
class DDMetric:
    ...
    def dist_fenics(self, z_mech, z_db = None):
        if(type(z_db) == type(None) ):
            self.deps.vector.array[:] = z_mech[0].vector.array[:]
            self.dsig.vector.array[:] = z_mech[1].vector.array[:]
        else:
            self.deps.vector.array[:] = z_mech[0].vector.array[:] - z_db[0].vector.array[:]
            self.dsig.vector.array[:] = z_mech[1].vector.array[:] - z_db[1].vector.array[:]
            
        return ft.L2norm_given_form(self.a_metric, self.comm)
    ...
    def norm_fenics(self, z): # receives a list of DDFunction
        return ft.L2norm_given_form(fem.form(self.form_inner_prod(z.as_vector())), self.comm)     
    ...
    def norm_energy_fenics(self, z):
        return ft.L2norm_given_form(fem.form(self.form_energy(z)), self.comm)  

    
    def distance_distTree(self, distTree):
        self.dist_func.vector.array[:] = distTree.flatten()
        return  ft.L2norm_given_form(fem.form(self.dist_func**2*self.dx), self.comm) # L2 norm
