...
def comparison_with_reference_sol(sol, sol_ref = None, output_sol_ref = None, labels = ["uh", "eps_mech", "sig_mech"], Vref = None, dxm = None): # Comparison
    ...
    if(not Vref):
        Vref = dd.DDSpace(Uh, dim = dim, representation = 'DG') # for stress          
        dxm = Vref.dxm
    
    norm = lambda x : ft.L2norm(x, dx = dxm, comm = mesh.comm)
    
    if(output_sol_ref):
        sol_ref = {"state" : [dd.DDFunction(Vref), dd.DDFunction(Vref)],
                    "u" : fem.Function(sol['u'].function_space) }

        with io.XDMFFile(output_sol_ref) as f:
